...
dict_por_fecha = dict() #para el csv que se usarÃ¡ para exportar el reporte a excel :(
...
claves_habitaciones_disp = list()   #parece que no tiene uso, checar eso
...
while True:
    ...
    print("5. Salir.")
    opcion= int(input("Selecionar una opciÃ³n: "))
    ...
    if opcion == 1:
        while True:
            ...
            print("***********************************************************")
            print("1. Registrar nueva reservaciÃ³n.")
            print("2. Modificar descripciÃ³n de una reservaciÃ³n.")
            print("3. Consultar disponibilidad de salas para un fecha.")
            print("4. Eliminar una reservaciÃ³n.")
            print("5. Volver al menÃº principal.")
            opcion2= int(input("Selecionar una opciÃ³n: "))
            if opcion2 == 1:
                print("\n**********************************************************")
                print("**              RESERVACIÃ“N DE UNA SALA                 **")
                print("**********************************************************")
                try:
                ...
                if (len(clientes_registrados) == 0):
                    print("\nNo se encontraron clientes registrados, registrate como cliente para hacer una reservaciÃ³n.")
                else:
                    print("\nNumero\tNombre")
                    print("*" * 30)
                    for numero, nombre in clientes_registrados:
                        print(f"{numero}\t{nombre}")
                    numero_capturado = int(input("\nIntroduce tu nÃºmero de cliente para hacer una reservaciÃ³n: "))
                    try: 
                        with sqlite3.connect("Renta_espacios.db") as conexion:
                            mi_cursor = conexion.cursor()
                    ...
                    if numero_capturado in set_nums_clientes:
                            ...
                            with sqlite3.connect("Renta_espacios.db") as conexion:
                                ...
                                if salas_registradas:
                                    ...
                                    while sala_deseada not in set_claves_salas:
                                        ...
                                        while (delta.days < 2):
                                            print("\nLa reservaciÃ³n tiene que ser, por lo menos, dos dÃ­as antes de la fecha actual.\n")
                                            fecha_capturada = input("Introduce la fecha del evento (DD/MM/AAAA): ")
                                            fecha_procesada = datetime.datetime.strptime(fecha_capturada, "%d/%m/%Y").date()
                                            fecha_evento = datetime.datetime.combine(fecha_procesada, datetime.time(00, 00, 00))
                                            delta = fecha_evento - rounded_actual
                                        ...
                                        while (turno_evento == "") or (turno_evento.isspace() == True) or turno_evento not in ("M", "V", "N"):
                                            print("\n-- Se debe ingresar un turno vÃ¡lido para la reservaciÃ³n --")
                                            turno_evento = input("Ingresa el turno en el que quieres reservar la sala (M (matutino)/ V (vespertino) / N (nocturno): ").upper()
                                        with sqlite3.connect("Renta_espacios.db") as conn:
                                                ...
                                                if salas_ocupadas:
                                                    print("\nYa existe una reservaciÃ³n para esa sala, a esa fecha y en ese turno.")
                                                else:
                                                    try:
                                                        with sqlite3.connect("Renta_espacios.db") as conn:
                                                            mi_cursor = conn.cursor()
                        ...
                        print("\nEl nÃºmero de cliente no existe, tienes que registrarte para hacer una reservaciÃ³n.\n")
            elif opcion2 == 2:
                print("\n**********************************************************")
                print("**         DESCRIPCIÃ“N DE UNA RESERVACIÃ“N               **")
                print("**********************************************************")
                try:
                    with sqlite3.connect("Renta_espacios.db") as conexion:
                        mi_cursor = conexion.cursor()
                            ...
                            if evento_con_folio:                    
                                ...
                                if opcion_eliminar == 1:
                                    ...
                                    if (delta_eliminar.days < 3):
                                        print("\nSolo se pueden eliminar reservaciones con, por lo menos, tres dÃ­as de anticipaciÃ³n.")
                                    else:
                                        try:
                                            mi_cursor.execute("DELETE FROM evento WHERE folio = ?", tupla_folio_eliminar)
                                            print(f"\nSe ha eliminado la reservaciÃ³n con el folio {folio_eliminar}.")
                                        except sqlite3.Error as e:
                                            print(e)
                                        except:
                                            print(f"Se produjo el siguiente error: {sys.exc_info()[0]}")
                                else:
                                    print("\nSe ha suspendido la operaciÃ³n.")
                            else:
                                print("\nNo se encontrÃ³ una reservaciÃ³n con ese folio.")
                    except sqlite3.Error as e:
                        print(e)
                    except:
                        print(f"Se produjo el siguiente error: {sys.exc_info()[0]}")
        ...
        while True:
            ...
            opcion3= int(input("Selecionar una opciÃ³n: "))
            if opcion3 == 1:
                ...
                try:
                    with sqlite3.connect("Renta_espacios.db", detect_types = sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as conexion:
                        ...
                        if eventos_en_fecha:
                            print("\n---------------------------------------------------------------------")
                            print(f"--    REPORTE DE RESERVACIONES PARA EL DÃA {fecha_proc_consulta}  --")
                            print("---------------------------------------------------------------------")
        ...
        while (cupo_sala == "0" or str.isspace(cupo_sala) or cupo_sala == "" or "-" in cupo_sala):
            print("\n-- Ingresa un cupo de sala vÃ¡lido --")
            cupo_sala = input("Ingresa el cupo de la sala: ")
        try:
            with sqlite3.connect("Renta_espacios.db") as conexion:
                mi_cursor = conexion.cursor()
        ...
        try:
            with sqlite3.connect("Renta_espacios.db") as conexion:
                ...
                print(f"\nEl nÃºmero de cliente para {nombre_cliente} es {mi_cursor.lastrowid}\n")
        except sqlite3.Error as e:
            print (e)
        except:
            print(f"Se produjo el siguiente error: {sys.exc_info()[0]}")
